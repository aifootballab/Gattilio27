# Verifica stili di gioco eFootball – confronto con fonti ufficiali

**Data**: 2 Febbraio 2026  
**Fonti**: Game8, SportsDunia, CONTENUTI_INTERNET_DA_INTEGRARE.md

## Lista completa Game8 (22 stili)

| EN (Game8) | IT (info_rag) | Stato |
|------------|---------------|-------|
| Poacher | Opportunista | ✅ |
| Adv. Striker | Punta avanzata | ✅ Aggiunto |
| Dummy Runner | Senza palla | ✅ |
| Fox in the Box | Rapace d'area | ✅ |
| Target Man | Fulcro di gioco | ✅ |
| Deep-Lying Forward | Punta arretrata | ✅ Aggiunto |
| Prolific Winger | Ala prolifica | ✅ |
| Roaming Flank | Taglio al centro | ✅ |
| Cross Specialist | Specialista di cross | ✅ |
| Classic No. 10 | Classico n° 10 | ✅ |
| Creative Playmaker | Regista creativo | ✅ |
| Hole Player | Giocatore chiave | ✅ |
| Box-to-Box | Box-to-Box | ✅ |
| The Destroyer (EN) | Incontrista | ✅ *(termine EN non usato in gioco IT)* |
| Orchestrator | Sviluppo | ✅ |
| Anchor Man | Collante | ✅ |
| Build Up | Sviluppo (CC/MED/DC) | ✅ |
| Extra Frontman | Frontale extra | ✅ |
| Offensive Fullback | Terzino offensivo | ✅ |
| Defensive Fullback | Terzino difensivo | ✅ |
| Fullback Finisher | Terzino mattatore | ✅ |
| Offensive Goalkeeper | Portiere offensivo | ✅ |
| Defensive Goalkeeper | Portiere difensivo | ✅ |

**Nota**: Onnipresente non compare nella lista Game8 ma è presente in eFootball (versione IT/altre). Box-to-Box e Onnipresente sono stili distinti.

## Stili IA (Con Palla) – 7 stili
Funambolo, Serpentina, Treno in corsa, Inserimento, Esperto palle lunghe, Crossatore, Tiratore – tutti presenti in info_rag.
