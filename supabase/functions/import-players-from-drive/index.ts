// @ts-nocheck
// Supabase Edge Function: Import Players from Google Drive
// Importa giocatori da file JSON su Google Drive in players_base
...
interface ImportRequest {
  ...
  json_data?: any // JSON diretto (per test)
  batch_size?: number
}
...
serve(async (req) => {
  ...
  try {
    const { google_drive_file_id, google_drive_url, json_data, batch_size = 50 }: ImportRequest = await req.json()
    ...
    if (json_data) {
      // JSON diretto
      players = Array.isArray(json_data) ? json_data : 
                json_data.players ? json_data.players :
                json_data.data ? json_data.data : []
    } else if (google_drive_file_id || google_drive_url) {
      // Carica da Google Drive
      const fileId = google_drive_file_id || extractFileId(google_drive_url)
      ...
      players = Array.isArray(jsonData) ? jsonData : 
                jsonData.players ? jsonData.players :
                jsonData.data ? jsonData.data : []
    } else {
      throw new Error('Must provide json_data, google_drive_file_id, or google_drive_url')
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        updated,
        errors,
        errorsList: errorsList.slice(0, 10) // Prime 10
      }),
...
function mapPlayerData(jsonPlayer: any) {
  // Estrai stats (supporta vari formati)
  ...
  return {
    ...
    age: jsonPlayer.age || jsonPlayer.eta || jsonPlayer.age || null,
