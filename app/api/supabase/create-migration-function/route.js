...
// ⚠️ ROUTE PER CREARE LA FUNZIONE DI MIGRAZIONE
// Chiama questa PRIMA, poi chiama /api/supabase/run-migration
...
export async function POST(req) {
  try {
    ...
    // Dobbiamo usare una connessione diretta al database o SQL Editor
    
    // Metodo alternativo: usa fetch diretto con Management API se disponibile
    ...
    // Prova con rpc exec_sql se esiste
    try {
      const { error } = await admin.rpc('exec_sql', { sql: createFunctionSQL })
      if (error) {
        return NextResponse.json({
          error: 'Cannot create function via API',
          details: error.message,
          hint: 'Esegui create_migration_function.sql in SQL Editor, poi chiama /api/supabase/run-migration'
        }, { status: 500 })
      }
      
      return NextResponse.json({
        success: true,
        message: 'Funzione migrate_to_players_table() creata!',
        next: 'Chiama /api/supabase/run-migration per eseguire la migrazione'
      })
    } catch (e) {
      return NextResponse.json({
    ...
    console.error('[create-migration-function] Error:', e)
    return NextResponse.json(
      { 
        error: e?.message || 'Errore creazione funzione',
        hint: 'Esegui create_migration_function.sql in SQL Editor, poi chiama /api/supabase/run-migration'
      },
      { status: 500 }
    )
