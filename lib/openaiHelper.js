...
export async function callOpenAIWithRetry(apiKey, requestBody, operationType = 'openai-call') {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.log(`[${operationType}] OpenAI error: status=${statusCode}, code=${errorCode}, message=${errorData.error?.message || ''}, model requested=${requestBody?.model || '?'}`)
      ...
      const isModelError = statusCode === 404 || errorCode === 'model_not_found' ||
