...
export async function syncBalanceCache(admin, userId, calculatedData) {
  try {
    ...
    if (existing) {
      ...
      if (!error && data) {
        console.log(`[heroPointsHelper] DEBUG UPDATE executed: set balance=${updateData.hero_points_balance}, total_purchased=${updateData.total_purchased}, got back balance=${data.hero_points_balance}, total_purchased=${data.total_purchased}`)
        // Se il valore restituito non corrisponde, c'è un problema
        if (data.hero_points_balance !== calculatedData.balance) {
          console.error(`[heroPointsHelper] CRITICAL: UPDATE returned wrong balance! Expected ${calculatedData.balance}, got ${data.hero_points_balance}`)
        }
        if (data.total_purchased !== calculatedData.totalPurchased) {
          console.error(`[heroPointsHelper] CRITICAL: UPDATE returned wrong total_purchased! Expected ${calculatedData.totalPurchased}, got ${data.total_purchased}`)
        }
      }
    } else {
      // INSERT nuovo record
