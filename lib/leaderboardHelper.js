 ...
 * Uso: solo server-side (API con service role). Breakdown: solo le voci che contribuiscono al totale.
 */
...
export async function computeLeaderboardForMonth(month, admin) {
  ...
  // Tutti i profili (nessun filtro consenso: tutti gli eleggibili entrano in classifica)
  const { data: profiles } = await admin
    .from('user_profiles')
    .select('user_id, nickname, profile_completion_score')
