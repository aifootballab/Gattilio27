...
const MIN_MATCHES_ELIGIBILITY = 3
const MIN_TASKS_ELIGIBILITY = 1
...
/**
 * Upsert snapshot classifica nel DB (solo server)
 */
export async function saveLeaderboardSnapshot(month, rankings, admin) {
  ...
  for (const row of rows) {
    await admin.from('leaderboard_snapshots').upsert(row, { onConflict: 'month,user_id', ignoreDuplicates: false })
  }
}
