 ...
 * Mostra debolezze, punti di forza e lezioni apprese dalla conversazione di feedback.
 */
function buildCoachFeedbackSection(feedbackRows, lang) {
  ...
  for (const row of feedbackRows) {
    ...
    const opp = row.opponent_name ? ` (vs ${sanitizeForPrompt(row.opponent_name, 30)}${row.outcome ? `, ${row.outcome}` : ''})` : ''
    for (const ins of insights) {
      if (!ins || !ins.text) continue
      const typeLabel = ins.type === 'weakness'
        ? (lang === 'en' ? 'Weakness' : 'Debolezza')
        : ins.type === 'strength'
          ? (lang === 'en' ? 'Strength' : 'Forza')
          : (lang === 'en' ? 'Lesson' : 'Lezione')
      lines.push(`- ${typeLabel}: ${sanitizeForPrompt(ins.text, 150)}${opp}`)
    }
  }
  if (lines.length === 0) return ''
  const count = feedbackRows.length
  const header = lang === 'en'
    ? `Last 30 days, ${count} session${count > 1 ? 's' : ''}`
    : `Ultimi 30 giorni, ${count} session${count > 1 ? 'i' : 'e'}`
  return `${header}:\n${lines.join('\n')}`
}
