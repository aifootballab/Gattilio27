...
function calculateRosterScore(players, formation) {
  ...
  // Bonus: Dati completi (overall_rating, posizioni) = 5%
...
function calculateCoachScore(coach) {
  ...
  // Allenatore caricato e attivo = 10% (colonna DB: coach_name)
...
/**
 * Calcola score componente Utilizzo Sistema (max 5% — ridotto da 10% per fare spazio a coach_training)
...
/**
 * Calcola score componente Coach Training — Palestra Coach (max 10%)
...
/**
 * Calcola score componente Successi & Obiettivi (max 10% — ridotto da 15% per fare spazio a coach_training)
...
export async function calculateAIKnowledgeScore(userId, supabaseUrl, serviceKey) {
  ...
  try {
    ...
    // Pesi: Profile 20% + Roster 25% + Matches 30% + Patterns 15% + Coach 10% + Usage 5% + Success 10% + CoachTraining 10% = 125% (cap 100)
