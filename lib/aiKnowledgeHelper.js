...
function calculateSuccessScore(profile, weeklyGoals, matches) {
  ...
  // FIX: Considera solo obiettivi completati nelle ultime 2 settimane per score rilevante
  if (weeklyGoals && Array.isArray(weeklyGoals)) {
    const now = new Date()
    const twoWeeksAgo = new Date(now.getTime() - 14 * 24 * 60 * 60 * 1000)
    
    const completedGoals = weeklyGoals.filter(g => {
      if (g.status !== 'completed') return false
      // Verifica completato recente (entro 2 settimane)
