...
function calculatePatternsScore(tacticalPatterns) {
  ...
  // Se esistono pattern identificati, score base 10%
  let score = 0
  
  // Pattern identificati = 10%
  if (Object.keys(tacticalPatterns).length > 0) {
    score = 10
  }
  
  // Bonus: Pattern confermati (5%)
  // TODO: Implementare logica pattern confermati quando disponibile
...
export async function calculateAIKnowledgeScore(userId, supabaseUrl, serviceKey) {
  ...
  try {
    ...
    // 5. Fetch Pattern Tattici (se disponibili)
    // TODO: Implementare quando tabella pattern sarà disponibile
    ...
    // 8. Calcolo Utilizzo Sistema (stima basata su partite e chat)
    // TODO: Implementare tracking utilizzo quando disponibile
    const usage = {
      chat_messages: 0, // TODO: Tracciare messaggi chat
      interactions: (matches?.length || 0) + (players?.length || 0)
    }
    ...
    const patternsScore = calculatePatternsScore(tacticalPatterns)
