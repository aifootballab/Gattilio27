...
export function getRelevantSections(userMessage, maxChars = DEFAULT_MAX_CHARS) {
  ...
  // Enterprise: includi sempre "NOTE CRITICHE PER L'IA" se non già presente (regole e errori comuni)
  const NOTE_CRITICHE_TITLE = "10. NOTE CRITICHE PER L'IA"
  const hasNoteCritiche = selected.some(s => s.title === NOTE_CRITICHE_TITLE)
  if (selected.length > 0 && !hasNoteCritiche) {
    const noteSection = sections.find(s => s.title === NOTE_CRITICHE_TITLE)
    if (noteSection && total + noteSection.content.length <= maxChars) {
...
const PERSONAL_CONTEXT_TERMS = [
  ...
  'squadra', 'mia squadra', 'la squadra', 'della mia squadra', 'sguarda',
  'partite caricate', 'partite salvate', 'quante partite', 'ultime partite', 'ultima partita',
  'partite', 'nelle partite', 'come sono andato', 'andamento partite', 'come è andata',
  'risultati', 'com\'è andata', "com'e andata", 'ultimi risultati', 'partita vinta', 'partita persa',
  'ho vinto', 'ho perso', 'nell\'ultima', "nell'ultima",
  'come pensi', 'pensi che sia', 'che ne pensi', 'come è la mia rosa', 'come va la mia',
  ...
  'che modulo', 'quale modulo', 'modulo migliore', 'formazione migliore', 'consigli modulo',
  'che formazione', 'quale formazione uso', 'formazione devo usare', 'formazione mi consigli',
  ...
  'how did i do', 'how are my matches', 'did i win', 'last match',
  'recommend', 'recommendations', 'what do you recommend', 'suggest', 'suggestions',
  'which formation', 'who should i play', 'who to put', 'best lineup',
  'cosa mi consigli', 'consigli sulla', 'consiglio tecnico', 'consigli tecnici',
  'cambiare della', 'cosa cambiare', 'cosa consigli di cambiare',
  'mi consigli', 'dammi consigli', 'consigli per', 'consigli su', 'suggerimenti per',
  ...
  'difficoltà', 'mia difficoltà', 'difesa debole', 'prendo goal', 'aiuto tattica', 'consigli per vincere',
  'che difensori ho', 'quali difensori', 'dimmi che difensori', 'chi ho in difesa',
  'cosa fare per', 'cosa dovrei', 'come devo impostare', 'impostare la squadra'
]
