 ...
 */
function translate(key, lang = 'it', params = {}) {
  const safeLang = LANG_WHITELIST.includes(lang) ? lang : 'it'
  const translation = translations[safeLang]?.[key] || translations.en?.[key] || key
...
export async function generateWeeklyTasksForUser(userId, supabaseUrl, serviceKey, week, lang = 'it') {
  ...
  try {
    ...
    const tasks = await generateTasksBasedOnData(profile, matches || [], patterns, safeLang)
    ...
    if (!tasks || tasks.length === 0) {
      console.warn(`[TaskHelper] No tasks generated, falling back to generic tasks`)
      const genericTasks = await generateTasksBasedOnData(null, [], null, safeLang)
      if (genericTasks && genericTasks.length > 0) {
        tasks.push(...genericTasks)
        console.log(`[TaskHelper] Added ${genericTasks.length} generic tasks as fallback`)
      } else {
        // Ultimo fallback: task hardcoded (solo tipi data-derived)
        tasks.push({
          goal_type: 'complete_matches',
          goal_description: translate('goalCompleteMatches', safeLang, { count: 3 }),
          target_value: 3,
          difficulty: 'easy'
        })
        tasks.push({
        ...
        tasks.push({
          ...
          goal_description: translate('goalUseAIRecommendations', safeLang, { count: 2 }),
          target_value: 2,
          difficulty: 'easy'
        })
        console.log(`[TaskHelper] Added 3 hardcoded generic tasks as last fallback`)
      }
    }
 ...
 */
async function generateTasksBasedOnData(profile, matches, patterns, lang = 'it') {
  const tasks = []
  const safeLang = LANG_WHITELIST.includes(lang) ? lang : 'it'
  ...
  // Task 4: Basato su problemi comuni (check case-insensitive: "Difesa" o "difesa")
  if (profile?.common_problems && Array.isArray(profile.common_problems)) {
    const hasDefense = profile.common_problems.some(p => String(p).toLowerCase() === 'difesa')
    if (hasDefense) {
      tasks.push({
        goal_type: 'improve_defense',
        goal_description: translate('goalImproveDefense', safeLang, { count: 2 }),
        target_value: 2,
        difficulty: 'easy'
      })
    }
...
export async function updateTasksProgressAfterMatch(userId, supabaseUrl, serviceKey, matchData) {
  ...
  try {
    if (!matchData.id) {
      console.warn('[TaskHelper] Invalid matchData (missing id) in updateTasksProgressAfterMatch')
      return []
    }
    ...
    const { data: recentMatches, error: matchesError } = await admin
      .from('matches')
      .select('id, match_date, result, team_stats, data_completeness, formation_played, recommended_formation_used')
      .eq('user_id', userId)
      .order('match_date', { ascending: false })
    ...
    if (activeTasks && activeTasks.length > 0) {
      ...
      const uniqueWeeks = Array.from(new Set(taskWeekDates.map(w => `${w.start}_${w.end}`)))
        .map(key => {
    ...
    for (const task of activeTasks) {
      try {
        ...
        const newProgress = await calculateTaskProgress(task, allMatches, matchData, admin, userId)
        ...
        if (progressChanged) {
          ...
          const roundedCurrent = Math.round(newProgress.current_value * 100) / 100
          ...
          const isCompleted = task.target_value > 0 &&
                              task.status === 'active' &&
                              (lowerIsBetter
          ...
          if (isCompleted) {
            console.log(`[TaskHelper] Task ${task.id} (${task.goal_type}) completed: ${roundedCurrent} ${lowerIsBetter ? '<=' : '>='} ${roundedTarget} (tolerance: ${tolerance})`)
    ...
    if (completedTasks.length > 0) {
      console.log(`[TaskHelper] ${completedTasks.length} task completati, avvio aggiornamento AI Knowledge...`)
      
      import('./aiKnowledgeHelper').then(async ({ updateAIKnowledgeScore }) => {
        try {
          const result = await updateAIKnowledgeScore(userId, supabaseUrl, serviceKey)
          console.log(`[TaskHelper] AI Knowledge aggiornato: score=${result.score}, level=${result.level}`)
        } catch (err) {
          console.error('[TaskHelper] Failed to update AI knowledge score:', err)
        }
      }).catch(err => {
        console.error('[TaskHelper] Failed to import aiKnowledgeHelper:', err)
      })
    }
...
/** Formazioni considerate difensive per obiettivo improve_defense (whitelist fissa, sicurezza) */
const DEFENSIVE_FORMATIONS = new Set(['5-3-2', '5-4-1', '3-5-2', '4-1-4-1', '5-2-2-1', '3-4-2-1', '4-2-3-1', '4-4-1-1'])

/** Whitelist description credit_transactions per use_ai_recommendations (solo da codice, mai da request) */
const AI_USAGE_DESCRIPTIONS_WHITELIST = ['assistant-chat', 'analyze-match', 'generate-countermeasures', 'extract-formation', 'extract-match-data', 'extract-game-analysis']
 ...
 */
async function calculateTaskProgress(task, recentMatches, newMatch, admin, userId) {
  // Validazione input
  ...
  switch (task.goal_type) {
    ...
    case 'increase_wins':
      ...
      const winsInTaskWeek = recentMatches
        .filter(m => {
          ...
          const matchDate = new Date(m.match_date)
          return matchDate >= taskWeekStart && matchDate <= taskWeekEnd && isWinForUser(m.result, m.is_home)
        }).length

      // Aggiungi nuova partita se è vittoria e nella settimana del task
      if (newMatch && newMatch.match_date && newMatch.result) {
        const newMatchDate = new Date(newMatch.match_date)
        if (newMatchDate >= taskWeekStart && newMatchDate <= taskWeekEnd && isWinForUser(newMatch.result, newMatch.is_home)) {
    ...
    case 'improve_defense': {
      const defensiveInWeek = recentMatches.filter(m => {
        if (!m || !m.match_date) return false
        const d = new Date(m.match_date)
        if (d < taskWeekStart || d > taskWeekEnd) return false
        const formation = (m.formation_played && String(m.formation_played).trim()) || ''
        return DEFENSIVE_FORMATIONS.has(formation)
      }).length
      if (newMatch && newMatch.match_date) {
        const newMatchDate = new Date(newMatch.match_date)
        if (newMatchDate >= taskWeekStart && newMatchDate <= taskWeekEnd) {
          const formation = (newMatch.formation_played && String(newMatch.formation_played).trim()) || ''
          if (DEFENSIVE_FORMATIONS.has(formation)) {
            currentValue = defensiveInWeek + 1
          } else {
            currentValue = defensiveInWeek
          }
        } else {
          currentValue = defensiveInWeek
        }
    ...
    case 'use_recommended_formation': {
      ...
      if (newMatch && newMatch.match_date && newMatch.recommended_formation_used === true) {
        const newMatchDate = new Date(newMatch.match_date)
        if (newMatchDate >= taskWeekStart && newMatchDate <= taskWeekEnd) {
    ...
    case 'use_ai_recommendations': {
      ...
      if (txError) {
        console.warn('[TaskHelper] credit_transactions query failed for use_ai_recommendations:', txError.message)
        currentValue = task.current_value || 0
      } else {
...
function calculateAvgGoalsConceded(matches) {
  ...
  matches.forEach(match => {
    ...
    if (typeof goalsConceded === 'number' && goalsConceded >= 0) {
      ...
      // Parse result: "6-1" → gol subiti dipendono da is_home (team1 = casa, team2 = trasferta)
      ...
      if (parts.length === 2) {
        ...
        if (!isNaN(team1Goals) && !isNaN(team2Goals) && team1Goals >= 0 && team2Goals >= 0) {
          if (match.is_home === true) {
            totalGoals += team2Goals  // Casa = team1, gol subiti = team2
          } else if (match.is_home === false) {
            totalGoals += team1Goals  // Trasferta = team2, gol subiti = team1
          } else {
            totalGoals += team2Goals  // Fallback dati vecchi (assume team1)
          }
          count++
        }
      }
...
/**
 * Vittoria per l'utente considerando is_home: casa = team1, trasferta = team2
 */
function isWinForUser(result, isHome) {
  if (!result || typeof result !== 'string') return false
  const upper = result.toUpperCase()
  ...
  if (isHome === true) return a > b   // Casa = team1, vinci se team1 > team2
  if (isHome === false) return b > a  // Trasferta = team2, vinci se team2 > team1
  return a > b  // Fallback: assume team1
}
...
/**
 * Sconfitta per l'utente considerando is_home
 */
function isLossForUser(result, isHome) {
  if (!result || typeof result !== 'string') return false
  const upper = result.toUpperCase()
  ...
  if (!/^\d+-\d+$/.test(result)) return false
  const [a, b] = result.split('-').map(s => parseInt(s, 10))
  if (isHome === true) return a < b
  if (isHome === false) return b < a
  return a < b
}
