  ...
  try {
    ...
    if (!tasks || tasks.length === 0) {
      ...
      if (genericTasks && genericTasks.length > 0) {
        ...
        // Ultimo fallback: task hardcoded
        tasks.push({
        ...
        tasks.push({
          goal_type: 'use_recommended_formation',
          goal_description: translate('goalUseRecommendedFormation', safeLang, { count: 1 }),
          target_value: 1,
          difficulty: 'easy'
        })
        tasks.push({
...
async function generateTasksBasedOnData(profile, matches, patterns, lang = 'it') {
  ...
  if (!matches || matches.length < 3) {
    ...
    // Task 2: Usa formazione consigliata
    tasks.push({
      goal_type: 'use_recommended_formation',
      goal_description: translate('goalUseRecommendedFormation', safeLang, { count: 1 }),
      target_value: 1,
    ...
    for (const task of activeTasks) {
      try {
        ...
        if (progressChanged) {
          ...
          // FIX: Aggiungi tolleranza per float e logging per debug
          ...
          if (isCompleted) {
            console.log(`[TaskHelper] Task ${task.id} (${task.goal_type}) completed: ${roundedCurrent} >= ${roundedTarget} (tolerance: ${tolerance})`)
...
const AI_USAGE_DESCRIPTIONS_WHITELIST = ['assistant-chat', 'analyze-match', 'generate-countermeasures', 'extract-formation', 'extract-match-data']
