...
export async function generateWeeklyTasksForUser(userId, supabaseUrl, serviceKey, week) {
  ...
  try {
    ...
    if (existingTasks && existingTasks.length > 0) {
      console.log(`[TaskHelper] Tasks already exist for week ${week.start}, skipping generation`)
      return []
    }
    ...
    if (tasks.length > 0) {
      ...
      console.log(`[TaskHelper] Generated ${insertedTasks.length} tasks for user ${userId}`)
...
async function generateTasksBasedOnData(profile, matches, patterns) {
  ...
  // Se non ci sono abbastanza dati, genera task generici
  if (!matches || matches.length < 3) {
    ...
    return tasks.slice(0, 3) // Max 3 task generici
  }
