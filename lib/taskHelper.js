...
function translate(key, lang = 'it', params = {}) {
  ...
  // Sostituisci parametri {param} con valori
...
export async function generateWeeklyTasksForUser(userId, supabaseUrl, serviceKey, week) {
  ...
  try {
    ...
    if (!tasks || tasks.length === 0) {
      ...
      const genericTasks = await generateTasksBasedOnData(null, [], null)
      if (genericTasks && genericTasks.length > 0) {
        ...
        tasks.push({
          ...
          goal_description: translate('goalCompleteMatches', 'it', { count: 3 }),
          target_value: 3,
          difficulty: 'easy'
        })
        tasks.push({
          goal_type: 'use_recommended_formation',
          goal_description: translate('goalUseRecommendedFormation', 'it', { count: 1 }),
          target_value: 1,
          difficulty: 'easy'
        })
        tasks.push({
          goal_type: 'use_ai_recommendations',
          goal_description: translate('goalUseAIRecommendations', 'it', { count: 2 }),
          target_value: 2,
          difficulty: 'easy'
        })
...
async function generateTasksBasedOnData(profile, matches, patterns) {
  ...
  if (avgGoalsConceded > 2.0) {
    ...
    if (target > 0 && target < avgGoalsConceded) {
      tasks.push({
        ...
        goal_description: translate('goalReduceGoalsConceded', 'it', { 
  ...
  if (avgPossession < 50 && avgPossession >= 0) {
    ...
    if (target > avgPossession && target <= 100) {
      tasks.push({
        ...
        goal_description: translate('goalImprovePossession', 'it', { 
  ...
  if (winRate < 50 && winRate >= 0) {
    tasks.push({
      ...
      goal_description: translate('goalIncreaseWins', 'it', { count: 3 }),
      target_value: 3,
      difficulty: 'hard'
    })
  }
  ...
  if (profile?.common_problems && Array.isArray(profile.common_problems)) {
    ...
    if (hasDefense) {
      tasks.push({
        ...
        goal_description: translate('goalImproveDefense', 'it', { count: 2 }),
        target_value: 2,
        difficulty: 'easy'
      })
    }
