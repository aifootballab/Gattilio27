...
/** Whitelist description credit_transactions per use_ai_recommendations (stesse operazioni che consumano HP). Stessa lista in leaderboardHelper.USAGE_WHITELIST per classifica. */
const AI_USAGE_DESCRIPTIONS_WHITELIST = ['assistant-chat', 'analyze-match', 'generate-countermeasures', 'extract-formation', 'extract-match-data', 'extract-game-analysis', 'extract-player', 'extract-coach', 'coach-feedback-chat', 'save-coach-feedback']
  ...
  if (match.result && typeof match.result === 'string') {
    ...
    if (parts.length === 2) {
      ...
      if (!isNaN(team1Goals) && !isNaN(team2Goals)) {
        if (match.is_home === true) return team2Goals
        if (match.is_home === false) return team1Goals
        return team2Goals // fallback
      }
    }
  }
